...
let
    ...
    N=61      # System size
    cutoff=1E-9
    δτ=0.05   # trotter step
    beta=20.0 # 1/T
    NMETTS=20 # # of thermal step
    Nwarm=10 # # of warm up step

    Num_metts=100 # # of METTS (Samples)
    ...
    for j in 2:Num_metts
        energies[string(j)]=Float64[]
    end

    Num_lst = Dict(string(1)=>Float64[])
    for j in 2:Num_metts
        Num_lst[string(j)]=Float64[]
    end
    ...
    for times in 1:Num_metts
        ...
        # METTS have been made
            if step > Nwarm
                ...
                push!(energies[string(times)], real(energy))
                push!(Num_lst[string(times)], real(Nums))
            end
    ...
    file_name="Data_E_h_"*string(hz)*"_a_"*string(a)*"_L"*string(N)*"_mu_"*string(mu)*"_500_T_"*string(beta)*"_S_"*string(NMETTS)*"_Y_Z_"*string(mu_step)*".npz"
    npzwrite(file_name, energies)

    file_name="Data_N_h_"*string(hz)*"_a_"*string(a)*"_L"*string(N)*"_mu_"*string(mu)*"_500_T_"*string(beta)*"_S_"*string(NMETTS)*"_Y_Z_"*string(mu_step)*".npz"
